    ...
    with gr.Row(elem_id="models_tab"):
        ...
        with gr.Column(elem_id='models_input_container', scale=3):
            ...
            with gr.Tab(label="Huggingface"):
                ...
                def hf_download_model(hub_id: str, token, variant, revision, mirror, olive_optimized, custom_pipeline):
                    from modules.modelloader import download_diffusers_model
                    download_diffusers_model(hub_id, cache_dir=opts.olive_sideloaded_models_path if olive_optimized else opts.diffusers_dir, token=token, variant=variant, revision=revision, mirror=mirror, custom_pipeline=custom_pipeline)
                ...
                hf_download_model_btn.click(fn=hf_download_model, inputs=[hf_selected, hf_token, hf_variant, hf_revision, hf_mirror, hf_olive, hf_custom_pipeline], outputs=[models_outcome])
